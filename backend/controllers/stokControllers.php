        ...
        if (empty($_GET['s'])) {
            ...
            if (empty($_GET['entries'])) {
                ...
                if (empty($_GET['page'])) {
                    ...
                    $queryTotalRecords = "SELECT COUNT(*) total FROM barang";
            ...
            if (empty($_GET['entries'])) {
                ...
                if (empty($_GET['page'])) {
                    ...
                    $queryTotalRecords = "SELECT COUNT(*) FROM barang WHERE kd_barang LIKE ? OR nama_barang LIKE ?";
                    $stmtTotalRecords = $conn->prepare($queryTotalRecords);
                    $searchTerm = "%$search%";
                    $stmtTotalRecords->bind_param("ss", $searchTerm, $searchTerm);
                    $stmtTotalRecords->execute();
                    $resultTotalRecords = $stmtTotalRecords->get_result();

                    $totalRecords = $resultTotalRecords->fetch_assoc()['COUNT(*)'];
                    $totalPages = ceil($totalRecords / $entries);

                    $queryShowbarang = "SELECT * FROM barang WHERE kd_barang LIKE ? OR nama_barang LIKE ? LIMIT ?, ?";
                    $stmtShowbarang = $conn->prepare($queryShowbarang);
                    $stmtShowbarang->bind_param("ssii", $searchTerm, $searchTerm, $start_from, $entries);
                    $stmtShowbarang->execute();
                    $result = $stmtShowbarang->get_result();
