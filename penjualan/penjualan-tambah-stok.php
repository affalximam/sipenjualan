...
<?php $redirect = "penjualan-list.php"; ?>
<?php $page = "penjualan-tambah-stok"; ?>
<?php include ('../backend/connect/conn.php'); ?>
<?php include ('../backend/controllers/sessionController.php'); ?>
<?php include ('../backend/controllers/penjualanController.php');?>
...
<head>
    ...
    <title><?= htmlspecialchars($sessionUser); ?> - PENJUALAN - TAMBAH STOK</title>
...
<body class="row">
        ...
        <a href="" class="sidebar-title py-3 text-white text-center text-decoration-none bg-template-blue-2 fs-3">
            ...
            <ul class="nav flex-column mb-auto">
                ...
                <?php if ($sessionLayoutType == 1) : ?>
                    <li>
                        <a href="../penjualan/" class="nav-link active text-white border-bottom py-3 ps-4 text-white">
                    ...
                    <li>
                        <a href="../pelanggan/" class="nav-link text-white border-bottom py-3 ps-4 text-white">
                ...
                <?php elseif( $sessionLayoutType == 2) : ?>
                    <li class="nav-link nav-link-dropdown text-white border-bottom py-3 ps-4">
                        <div class="d-flex justify-content-between">
                            <a href="#" class="text-white m-0 p-0 text-decoration-none">
    ...
    <main class="main col-12 col-md-12 col-lg-9 col-xxl-10 p-0">
        ...
        <section class="content p-5 pt-1 bg-secondary-subtle">
            <h1 class="my-2">PENJUALAN <span class="fs-4 text-secondary">TAMBAH STOK T-01</span></h1>
            <?php if ($transaksi == 1) {?>
                <div class="bg-success py-4 my-3 text-white text-center rounded-2">
                    <h2>Transaksi Baru Berhasil Ditambah</h2>
                </div>
            <?php } ?>
            <div class="bg-light p-2 pt-3 pb-5">
                <div class="container rounded-2 container-type-1">
                    ...
                    <div class="row">
                        <p class="fs-5 m-0 fw-bold">KODE PENJUALAN : 
                            <span class="fw-normal">
                                <?= htmlspecialchars($_GET['kd_penjualan']); ?>
                            </span>
                        </p>
                        <p class="fs-5 m-0 fw-bold">NAMA PELANGGAN : 
                            <span class="fw-normal">
                                <?php $nama_pelanggan = getNamaPelanggan($conn, $id_pelanggan = intval($_GET['id_pelanggan'])); ?>
                                <?= htmlspecialchars($nama_pelanggan); ?>
                            </span>
                        </p>
                        <p class="fs-5 m-0 fw-bold">NAMA KASIR : <span class="fw-normal"><?= htmlspecialchars($_GET['kasir']); ?></span></p>
                    </div>
                    
                    <div class="row px-4">
                        <div class="container rounded-2 container-type-1">
                            <form method="POST" class="row py-2 px-3">
                                <input type="hidden" name="id_pelanggan" value="<?= htmlspecialchars($_GET['id_pelanggan']); ?>">
                                <input type="hidden" name="kd_penjualan" value="<?= htmlspecialchars($_GET['kd_penjualan']); ?>">
                                <input type="hidden" name="kasir" value="<?= htmlspecialchars($_GET['kasir']); ?>">
                                <hr>
                                <div class="col-sm-6">
                                    <div class="row">
                                        <div class="col-3">
                                        ...
                                        <div class="col-8">
                                            <select class="form-select" name="id_barang" required>
                                                <?php foreach($dataShowBarang as $key => $value) : ?>
                                                    <?php $id_barang = intval($value['id']); ?>
                                                    <?php $jumlah_barang = getStokBarang($conn, $id_barang); ?>
                                                    
                                                    <?php if($jumlah_barang < 1) { ?>
                                                        <option value="<?= htmlspecialchars($value['id']); ?>" disabled>
                                                            <?= htmlspecialchars($value['nama_barang']); ?> 
                                                            ( KOSONG )
                                                            <?= htmlspecialchars($value['harga_barang']); ?>
                                                        </option>
                                                        <?php }  else { ?>
                                                            <option value="<?= htmlspecialchars($value['id']); ?> ">
                                                            <?= htmlspecialchars($value['nama_barang']); ?> 
                                                            ( <?= htmlspecialchars($jumlah_barang); ?> )
                                                            <?= htmlspecialchars($value['harga_barang']); ?>
                                                        </option>
                                                    <?php } ?>
                                                <?php  endforeach; ?>
                                ...
                                <div class="col-sm-6">
                                    <div class="row">
                                        ...
                                        <div class="col-9">
                                            ...
                                            <button type="submit" name="tambah_transaksi_penjualan" class="d-none"></button>
                                        </div>
                                    </div>
                                </div>
                                <hr class="mt-2">
                            </form>
                            <?php if (!empty($_SESSION['penjualanData'])) : ?>
                                <div class="row pb-4 px-3">
                                    <button type="button" name="simpan_transaksi_penjualan_button" id="simpanTransaksiPenjualanButton" class="btn btn-md btn-primary w-100 bg-template-blue-3 border-0">
                    ...
                    <?php if (!empty($_SESSION['penjualanData'])) : ?>
                        <div class="row px-3 py-3 text-center">
                            ...
                            <table class="table text-center">
                                ...
                                <tbody>
                                    <form method="POST" class="formSimpanTransaksiPenjualan">
                                        <?php foreach ($_SESSION['penjualanData'] as $key => $value) { ?>
                                            <?php $id_barang = intval($value['id_barang']); ?>
                                            <?php $nama_barang = getNamaBarang($conn, $id_barang); ?>

                                            <tr>
                                                <td scope="row"><?= htmlspecialchars($nama_barang) ?></td>
                                                <td><?= htmlspecialchars($value['qty']) ?></td>
                                                <td><?= htmlspecialchars($value['subtotal_harga']) ?></td>
                                                <td><?= htmlspecialchars($value['total_harga']) ?></td>
                                            </tr>

                                            <input type="hidden" name="kd_penjualan[]" value="<?= htmlspecialchars($value['kd_penjualan']) ?>">
                                            <input type="hidden" name="id_pelanggan[]" value="<?= htmlspecialchars($value['id_pelanggan']) ?>">
                                            <input type="hidden" name="id_barang[]" value="<?= htmlspecialchars($value['id_barang']) ?>">
                                            <input type="hidden" name="subtotal_harga[]" value="<?= htmlspecialchars($value['subtotal_harga']) ?>">
                                            <input type="hidden" name="qty[]" value="<?= htmlspecialchars($value['qty']) ?>">
                                            <input type="hidden" name="total_harga[]" value="<?= htmlspecialchars($value['subtotal_harga']) ?>">
                                            <input type="hidden" name="kasir[]" value="<?= htmlspecialchars($value['kasir']) ?>">
                                        <?php } ?>
                                        <button type="submit" name="simpan_transaksi_penjualan" id="simpanTransaksiPenjualan" class="d-none">aa</button>
                                    </form>
                                </tbody>
                            </table>
                        </div>
