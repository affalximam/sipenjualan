...
<head>
    ...
    <title><?= htmlspecialchars($sessionUser); ?> - PENJUALAN - LIST</title>
...
<body class="row">
    ...
    <main class="main col-12 col-md-12 col-lg-9 col-xxl-10 p-0">
        ...
        <section class="content p-5 pt-1 bg-secondary-subtle">
            <h1 class="my-2">PENJUALAN <span class="fs-4 text-secondary">LIST</span></h1>
            <div class="bg-light p-2 pt-3 pb-5">
                <div class="container rounded-2 container-type-1">
                    <?php if ($sessionLayoutType == 1) : ?>
                        <div class="row">
                            <div class="col-12 d-flex justify-content-end">
                                <a href="penjualan-tambah.php" class="btn btn-md btn-primary bg-template-blue w-auto border-0 rounded-2 mt-3 me-5">Tambah Penjualan</a>
                            </div>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="row pt-4">
                        <div class="col-md-6 d-flex flex-row mb-3">
                            <label for="entries" class="form-label m-0 align-self-center">Show </label>
                            <form id="entryForm" method="GET">
                                <select class="form-select w-auto mx-2" id="entries" name="entries" aria-label="Show entries">
                                <option value="" disabled selected><?= htmlspecialchars($entries); ?></option>
                            ...
                            <label for="entries" class="form-label m-0 align-self-center"> entries</label>
                        </div>
                        <div class="col-md-6 d-flex justify-content-xl-end">
                            <form class="d-flex" method="GET" action="?entries=<?= htmlspecialchars($entries); ?>">
                                <label for="search" class="form-label m-0 pe-2 align-self-center">Search</label>
                                <input class="form-control me-2" id="search" type="number" placeholder="Search" aria-label="Search" name="s">
                            </form>
                        </div>
                    </div>

                    <div class="row row-table ms-2 pe-3">
                        <table class="table table-striped table-bordered mt-3 w-100 text-center">
                            <thead>
                                <tr>
                                    <th scope="col-1">No</th>
                                    <th scope="col-2">Kode Penjualan</th>
                                    <th scope="col-2">Tanggal</th>
                                    <th scope="col-2">Nama Pelanggan</th>
                                    <th scope="col-2">Nama Kasir</th>
                                    <th scope="col-1">Jumlah Barang</th>
                                    <th scope="col-1">Total</th>
                                    <th scope="col-1">Lihat/Hapus</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if($showPenjualan == 1) { ?>
                                    <?php foreach ($dataPenjualanList as $index => $row) : ?>
                                        <?php $id_pelanggan = htmlspecialchars($row['id_pelanggan']); $namaPelanggan = getNamaPelanggan($conn, $id_pelanggan); ?>
                                        <?php $kd_Penjualan = htmlspecialchars($row['kd_penjualan']); $totalPenjualan = getTotalPenjualan($conn, $kd_Penjualan); ?>
                                        <?php $kd_Penjualan = htmlspecialchars($row['kd_penjualan']); $totalharga = getTotalHarga($conn, $kd_Penjualan); ?>
                                        <tr>
                                            <th scope="row"><?= htmlspecialchars($start_from + $index + 1 ); ?></th>
                                            <td><?= htmlspecialchars($row['kd_penjualan']); ?></td>
                                            ...
                                            <td><?= htmlspecialchars($totalPenjualan); ?></td>
                                            <td><?= htmlspecialchars($totalharga); ?></td>
                                            <td  class="d-flex flex-row justify-content-center">
                                                <a href="penjualan-detail.php?kd_penjualan=<?= htmlspecialchars($row['kd_penjualan']); ?>" class="btn p-0 m-0 align-content-center text-primary">
                                                ...
                                                <?php if ($sessionLayoutType == 1) : ?>
                                                    <form method="POST">
                                                        <input type="hidden" name="kd_penjualan" value="<?= htmlspecialchars($kd_Penjualan);?>">
                                                        <button type="submit" name="hapus_Penjualan" class="btn p-0 m-0 align-content-center text-primary ms-2">
                                    ...
                                    <?php } else { ?>
                                    <tr>
                                        <th colspan="8" class="py-4">TIDAK ADA DATA</th>
                                    </tr>
                                <?php } ?>
                                </tbody>
                        </table>
                    ...
                    <div class="row py-3">
                        <div class="col-md-6">
                        <p>Showing <?= htmlspecialchars($start_from + 1); ?> to <?= htmlspecialchars($start_from + $entries); ?> of <?= htmlspecialchars($totalRecords); ?> entries</p>
                        </div>
                        <div class="col-md-6 d-flex justify-content-end">
                            <?php echo generatePaginationButtons($totalPages, $page); ?>
                        </div>
                    </div>
                </div>
            </div>
